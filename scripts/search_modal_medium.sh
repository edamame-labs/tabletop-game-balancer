#!/usr/bin/env bash
# Medium-fidelity search on Modal. Fast scores rank EK configurations weakly,
# so search medium directly. Nori surrogate
...
while true; do
  ...
  python -u -m ttbalance --backend modal --workers "$WORKERS" --run-type "$RUN_TYPE" \
    ...
    >> "$LOG" 2>&1
  python -u -m ttbalance --backend modal best >> "$LOG" 2>&1
  sleep 2
done
